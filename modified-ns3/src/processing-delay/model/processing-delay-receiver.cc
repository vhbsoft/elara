...
#include "processing-delay-receiver.h"
...
NS_LOG_COMPONENT_DEFINE ("ProcessingDelayReceiver");
NS_OBJECT_ENSURE_REGISTERED (ProcessingDelayReceiver);

TypeId
ProcessingDelayReceiver::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ProcessingDelayReceiver")
    .SetParent<Application> ()
    .AddConstructor<ProcessingDelayReceiver> ()
    .AddAttribute ("UdpPort",
                   "Port on which we listen for incoming probe packets.",
                   UintegerValue (9),
                   MakeUintegerAccessor (&ProcessingDelayReceiver::m_udpPort),
                   MakeUintegerChecker<uint16_t> ())
    .AddAttribute ("TcpPort",
                   "TcpPort on which we listen for incoming tcp connections.",
                   UintegerValue (10),
                   MakeUintegerAccessor (&ProcessingDelayReceiver::m_tcpPort),
                   MakeUintegerChecker<uint16_t> ())
...
ProcessingDelayReceiver::ProcessingDelayReceiver()
{
...
void
ProcessingDelayReceiver::SetMode (uint8_t mode)
{
...
void
ProcessingDelayReceiver::SetLogFileName (std::string name)
{
...
void
ProcessingDelayReceiver::DoDispose (void)
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
ProcessingDelayReceiver::StartApplication (void)
{
  NS_LOG_FUNCTION_NOARGS ();
  ...
  m_udpSocket->SetRecvCallback (MakeCallback (&ProcessingDelayReceiver::HandleUdpRead, this));
  ...
  if (m_mode == 0)
    {
      ...
      m_tcpSocket->SetAcceptCallback (
				      ...
				      MakeCallback (&ProcessingDelayReceiver::HandleTcpAccept, this)
				     );
      
      m_tcpSocket->SetCloseCallbacks (
				      MakeCallback (&ProcessingDelayReceiver::HandleTcpClose, this),
				      MakeCallback (&ProcessingDelayReceiver::HandleTcpError, this)
				     );
    }
}

void
ProcessingDelayReceiver::StopApplication ()
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
ProcessingDelayReceiver::HandleUdpRead (Ptr<Socket> socket)
{
...
void ProcessingDelayReceiver::HandleTcpAccept (Ptr<Socket> s, const Address& from)
{
  NS_LOG_FUNCTION (this << s << from);
  if (m_initialized == 0)
    s->SetRecvCallback (MakeCallback (&ProcessingDelayReceiver::Initialize, this));    
  else
    m_nextEvent = Simulator::Schedule (Seconds (0.0), &ProcessingDelayReceiver::Process, this);
  m_connection = s;  
}

void
ProcessingDelayReceiver::Initialize (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
...
void ProcessingDelayReceiver::HandleTcpError (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
}

void ProcessingDelayReceiver::HandleTcpClose (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
}

void
ProcessingDelayReceiver::Process (void)
{
  NS_LOG_FUNCTION (this);
