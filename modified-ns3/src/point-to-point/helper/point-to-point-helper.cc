...
//******ProcessingDelayCode***********//
  m_rxQueueFactory.SetTypeId ("ns3::DropTailQueue");
//******ProcessingDelayCode***********//
...
void 
PointToPointHelper::SetRxQueue (std::string type,
                              std::string n1, const AttributeValue &v1,
                              std::string n2, const AttributeValue &v2,
                              std::string n3, const AttributeValue &v3,
                              std::string n4, const AttributeValue &v4)
...
PointToPointHelper::InstallCoordinated (Ptr<Node> a, Ptr<Node> b, Ptr<PointToPointNetDevice> &netDev)
{
  ...
  Ptr<Queue> rxQueueA = m_rxQueueFactory.Create<Queue> ();
  devA->SetRxQueue (rxQueueA);
  Ptr<Queue> rxQueueB = m_rxQueueFactory.Create<Queue> ();
