...
//*****CompressionCode*****
  /**
   * Attach a CompressionModel to the PointToPointNetDevice.
   *
   */
  void SetCompressionModel (Ptr<CompressionModel> cm);
// *****CompressionCode*****/
//*****ProcessingDelayCode*****//
  /**
   * Attach a receiving queue to the PointToPointNetDevice.
   *
   * The PointToPointNetDevice "owns" a receiving queue that implements a queueing 
   * method such as DropTail or RED.
   *
   * @see Queue
   * @see DropTailQueue
   * @param queue Ptr to the new queue.
   */
  void SetRxQueue (Ptr<Queue> queue);  

  /**
   * Get a copy of the attached Receiving Queue.
   *
   * @returns Ptr to the queue.
   */
  Ptr<Queue> GetRxQueue (void) const;

  /**
   * Attach a ProcessingDelayModel to the PointToPointNetDevice.
   *
   */
  void SetProcessingDelayModel (Ptr<ProcessingDelayModel> pdm);

  /**
   * Called when a packet is done 'processing'
   */
  void ProcessComplete (Ptr<Packet> p);
//*****ProcessingDelayCode*****//
//*****PriorityQueueCode*****//
  /**
   * Called when a the txqueue is filled past the theshold.
   */
  void ProcessWait (Ptr<Packet> p);
   ...
   *
   */
  void SetPriorityQueueModel (Ptr<PriorityQueueModel> pqm);  
  Ptr<PriorityQueueModel> GetPriorityQueueModel (void) const; 
//*****PriorityQueueCode*****// 
...
//*****CompressionCode*****//
  /**
   * Compression model for send and receive packet events
   */
  Ptr<CompressionModel> m_compressionModel;
//*****CompressionCode*****//
//*****ProcessingDelayCode*****//
  /**
   * Processing delay model for receive packet events
   */
  Ptr<ProcessingDelayModel> m_processingDelayModel;
  /**
   * Queue to store receiving packets awaiting 'processing'
   */
  Ptr<Queue> m_rxQueue;
//*****ProcessingDelayCode*****//
//*****PriorityQueueCode*****//
  /**
   * PriorityQueue model for send and receive packet events
   */
  Ptr<PriorityQueueModel> m_priorityQueueModel;
//*****PriorityQueueCode*****//
};
