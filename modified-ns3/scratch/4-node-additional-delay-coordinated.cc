...
{   
  ...
  uint32_t thresholdPackets = 655349000;
  uint32_t thresholdBytes = 655349000;
...
//
  ...
  // Coordinated
  proc->EnableThreshold();
  proc->SetQueueMode(queueMode);
  if (queueMode == 'p') {
    proc->SetThresholdPackets(thresholdPackets);
  }
  else if (queueMode == 'b') {
    proc->SetThresholdBytes(thresholdBytes);
  }
  proc->SetTxQueue(netDev->GetQueue());
