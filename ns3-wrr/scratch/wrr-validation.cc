...
NS_LOG_COMPONENT_DEFINE ("WRR_Validation");
...
main (int argc, char *argv[])
{
  ...
  uint32_t DEFAULT_MEAN_PACKET_SIZE = tcp_adu_size + 54;
...
// creating the topology
  ...
  m_queueFactory.SetTypeId ("ns3::WeightedRoundRobinQueue");
  ...
  m_queueFactory.Set ("FirstMeanPacketSize", UintegerValue (firstMeanPacketSize));
  m_queueFactory.Set ("SecondMeanPacketSize", UintegerValue (secondMeanPacketSize));
  ...
  /*//Config::SetDefault ("ns3::TcpSocket::SegmentSize", UintegerValue (tcp_adu_size));
  Address R1Address (InetSocketAddress (N2R1_i.GetAddress (1), R1port));
  OnOffHelper onOffHelper1 ("ns3::TcpSocketFactory", R1Address);
  ...
  onOffHelper1.SetAttribute ("DataRate",StringValue ("0.5Mbps"));
  onOffHelper1.SetAttribute ("PacketSize",UintegerValue(200));
  onOffHelper1.SetAttribute ("MaxBytes",UintegerValue(ftp1_data_bytes));
  ApplicationContainer ftpApp1 = onOffHelper1.Install (S1.Get (0));
  ftpApp1.Start (Seconds (ftpApp1StartTime));
  ftpApp1.Stop (Seconds (ftpApp1EndTime));*/
  ...
  /*Address R2Address (InetSocketAddress (N2R2_i.GetAddress (1), R2port));
  OnOffHelper onOffHelper2 ("ns3::TcpSocketFactory", R2Address);
  ...
  onOffHelper2.SetAttribute ("DataRate",StringValue ("0.5Mbps"));
  //onOffHelper2.SetAttribute ("PacketSize",UintegerValue(100));
  onOffHelper2.SetAttribute ("MaxBytes",UintegerValue(ftp2_data_bytes));

  ApplicationContainer ftpApp2 = onOffHelper2.Install (S2.Get (0));
  ftpApp2.Start (Seconds (ftpApp2StartTime));
  ftpApp2.Stop (Seconds (ftpApp2EndTime));*/
  ...
  if (tracing)
    {
      ...
      p2p.EnableAscii (ascii.CreateFileStream ("wrr-r1.tr"), R1);
      p2p.EnableAscii (ascii.CreateFileStream ("wrr-r2.tr"), R2);
