...
TcpDecompression::push(int, Packet *p_in)
{
  ...
  // Only tcp traffic to port 9877 will be decompressed
  if (ntohs(tcp->th_dport) == 9877 && ZlibDecompression (start, buf, comp_size, uncomp_size))
    {
      ...
      memcpy((uint8_t*) p_out->transport_header()+20, buf, uncomp_size);
