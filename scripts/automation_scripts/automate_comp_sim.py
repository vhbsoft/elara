#Automated compression simulation for ./waf commands in NS-3
import csv
import os
...
#The below command is how to execute the compression sim using a ./waf command
...
compression_args['p0QueueSize'] = '4800' #Compressor Transmission queue size
...
compression_args['outputFile'] = './test_' 
...
while True:
	...
	for y in range(500, 6500, 500):
		...
		comp_cmds += ' --outputFile=' + compression_args['outputFile'] + str(bands[counter]) +'Mbps_' + '_num_of_packets_' + str(numPackets) + '_H_' + '.csv'
		...
		comp_cmds = './waf --run "'
		comp_cmds += 'compression-sim'
		comp_cmds += ' --outputFile=' + compression_args['outputFile'] + str(bands[counter]) +'Mbps_' + '_num_of_packets_' + str(numPackets) + '_L_' + '.csv'
		comp_cmds += ' --packetSize=' + compression_args['packetSize']
		comp_cmds += ' --s0p0DataRate=' + compression_args['s0p0DataRate']
		comp_cmds += ' --p1r0DataRate=' + compression_args['p1r0DataRate']
		comp_cmds += ' --compression=' + compression_args['compression']
