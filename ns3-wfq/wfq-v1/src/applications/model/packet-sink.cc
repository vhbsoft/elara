...
PacketSink::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::PacketSink")
	...
	.AddAttribute("TotalExpectedRx",
				  ...
				  UintegerValue(1),
				  MakeUintegerAccessor(&PacketSink::m_totalExpectedRx),
				  MakeUintegerChecker<uint32_t>())

	.AddAttribute("BandwidthInterval",
				  "Bandwidth Interval",
				  TimeValue(Seconds (0)),
				  MakeTimeAccessor(&PacketSink::m_bandwidthCalculationInterval),
				  MakeTimeChecker())

	.AddAttribute("ReceiverName",
				  "Receiver Name",
				  StringValue("R"),
				  MakeStringAccessor(&PacketSink::m_receiverName),
				  MakeStringChecker())
...
void PacketSink::StartApplication ()    // Called at time specified by Start
{
  ...
  {
	  m_bandwidth_Calculation_Event = Simulator::Schedule (m_bandwidthCalculationInterval, &PacketSink::CalculateBandwidth, this);
  }
  // *********************
...
//**************************************
void PacketSink::CalculateBandwidth()
{
	m_bandwidth_Calculation_Event = Simulator::Schedule (m_bandwidthCalculationInterval, &PacketSink::CalculateBandwidth, this);
